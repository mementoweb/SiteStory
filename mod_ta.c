 ...
 *  
 *
 *  Created by Lyudmila Balakireva on 6/15/09.
 *  Copyright 2009 LANL. All rights reserved.
 *
 */
#include "httpd.h"
...
module AP_MODULE_DECLARE_DATA ta_module;
...
static apr_status_t ta_out_filter3(ap_filter_t *f,apr_bucket_brigade *bb){
	...
	if (sctx  == NULL) {
		 ...
		 get_headers(f, r->headers_in); 
	         ...
	         ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta: in headers  request  %s", r->connection->remote_ip) ;
		 ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta: in headers  request  %s",sctx->headers) ;
	   
		const char *uri = r->unparsed_uri;
		ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta: in headers  request  %s",uri) ;
		...
		if ((status = apr_sockaddr_info_get(&sockaddr, conf->host, APR_INET, port, 0, r->pool)) != APR_SUCCESS) {
			 ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta:problem creating socket address"  ) ;
                         ap_remove_output_filter(f);
			 return ap_pass_brigade(f->next, bb) ;				
		}
		
		if ((status = apr_socket_create(&(sctx->sock), sockaddr->family, SOCK_STREAM, APR_PROTO_TCP, r->pool)) != APR_SUCCESS) {
			ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta:problem creating socket"  ) ;
                        ap_remove_output_filter(f);
			 return ap_pass_brigade(f->next, bb) ;
		}
		       
			if((status= apr_socket_opt_set(sctx->sock, APR_SO_NONBLOCK, 1 ))!= APR_SUCCESS) {
			ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta: problem setting nonblocking  "  ) ;
			ap_remove_output_filter(f);
			 return ap_pass_brigade(f->next, bb) ;
		        }
		...
		if ((status = apr_socket_timeout_set(sctx->sock, timeout)) != APR_SUCCESS) {
			ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta:setting socket timeout"  ) ;
			ap_remove_output_filter(f);
			 return ap_pass_brigade(f->next, bb) ;
		...
		if ((status = apr_socket_connect(sctx->sock, sockaddr)) != APR_SUCCESS) {
			ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta:problem connecting"  ) ;  
			ap_remove_output_filter(f);
			return ap_pass_brigade(f->next, bb) ;          
		}
		...
		//first chunk - client headers                                                                                                                                                                                                                                                                                                              
		  ...
		  ap_xlate_proto_to_ascii(chunk_hdr, hdr_len);
		      ...
		      ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta: fisrst chunk:%s", buf3);
                      cb = strlen (buf3);
		      ...
		      apr_size_t rr =0; 
			   ...
			   sent += rr;      
			   //ap_log_rerror(APLOG_MARK, APLOG_DEBUG, status, r,"mod_ta: bytes send header %d", sent ) ; 
                           } while (status != APR_TIMEUP && rr!=0 && sent < cb  ); 
                          if ((rr == 0 && sent < cb) || status == APR_TIMEUP||status!=APR_SUCCESS) { 
                           ap_log_rerror(APLOG_MARK, APLOG_DEBUG, status, r ,"mod_ta: Connection timed out first attempt" ) ;
			   ap_remove_output_filter(f);
                          return ap_pass_brigade(f->next, bb) ;  
			  }   
		...
		for (b = APR_BRIGADE_FIRST(bb); b != APR_BRIGADE_SENTINEL(bb); b = APR_BUCKET_NEXT(b)) {
			...
			ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,
						 "mod_ta:  %s (%s-%s): %" APR_SIZE_T_FMT " bytes",
						 f->frec->name,
		   ...
		   apr_size_t cb;	
		    ...
		    apr_status_t rv;
		     ...
		     char errmsg[256];
                     if (rv!=APR_SUCCESS) {
		       ap_log_rerror(APLOG_MARK, APLOG_DEBUG, rv, r,"mod_ta EOS: Connection timed out %s ",apr_strerror(rv, errmsg, sizeof errmsg)) ;
			...
			if (apr_bucket_read(b, &buf, &nbytes, APR_BLOCK_READ) == APR_SUCCESS) {
			          ...
			          if  (nbytes) {			
			            ...
			            apr_size_t hdr_len;
                                ...
                                ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta: length from nbytes:%" APR_SIZE_T_FMT , nbytes);   
			        hdr_len = apr_snprintf(chunk_hdr, sizeof(chunk_hdr),
						    "%" APR_UINT64_T_HEX_FMT CRLF, (apr_uint64_t)nbytes);
			        ap_xlate_proto_to_ascii(chunk_hdr, hdr_len);
			        status = apr_socket_send(sctx->sock,chunk_hdr,&hdr_len);
                              if ( status != APR_SUCCESS){
				ap_log_rerror(APLOG_MARK, APLOG_DEBUG, status, r,"mod_ta meta header section: Connection timed out" ) ;
				ap_remove_output_filter(f);
				return ap_pass_brigade(f->next, bb) ;
			      }
			      //ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta: next chunk header:%s", chunk_hdr);  
			          apr_size_t rr = 0; 
			          apr_size_t sent = 0;  
				  do { rr = nbytes - sent; 
				    status = apr_socket_send(sctx->sock,buf + sent ,&rr);
				    sent += rr; 
				    //  ap_log_rerror(APLOG_MARK, APLOG_DEBUG, status, r,"mod_ta data section: bytes send %d",sent);                                    
                                   } while (status != APR_TIMEUP && rr!=0 && sent < nbytes  ); 
				  if((rr == 0 && sent < nbytes) || status == APR_TIMEUP) {
                                   ap_log_rerror(APLOG_MARK, APLOG_DEBUG, status, r,"mod_ta data section: Connection timed out %s",apr_strerror(status, errmsg, sizeof errmsg)) ;
			            ap_remove_output_filter(f); 
				    return ap_pass_brigade(f->next, bb); 
				  }
                                 // ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta: next chunk header:%s", buf); 
				...
				rv = apr_socket_send(sctx->sock,ASCII_CRLF,&cb);
			         if (rv!= APR_SUCCESS){
			        ap_log_rerror(APLOG_MARK, APLOG_DEBUG, rv, r,"mod_ta end of chunk: Connection timed out %s",apr_strerror(rv, errmsg, sizeof errmsg) ) ;
			        ap_remove_output_filter(f);
			        return ap_pass_brigade(f->next, bb) ;
			        }
			  ...
			  ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,  "mod_ta:  %s (%s-%s): %s",  f->frec->name,
     ...
     static int check_excluded_dirs (void*    arg,request_rec *r ){
	  ...
	  if (filoc == -1 || (strcmp(r->uri, "/") == 0)) {
	    ...
	    first = apr_pstrcat(r->pool,"/",token,NULL);
            ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta:  dir excluded  request in if good  %s",first) ;
		...
		ex = 1;
	      ...
	      ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta: dir excluded  request in if  %s",s) ;
	      //   ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta:  hook  request in if  %s",uri) ;
	    }
	...
	static int myta_pre_conn(request_rec *r)
       {    srv = ap_get_server_name(r);
	    char *line;
            char *req_line;
            char *protocolstr;
	    conf = ap_get_module_config(r->per_dir_config, &ta_module);
	    // my_svr_cfg*    srv_conf = ap_get_module_config(r->server->module_config,&ta_module); 
	    // apr_status_t rv;
		...
		if(r->method_number == M_GET){ 
		  int ex=0;
		      ...
		      ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta:  excluded  flag %d",ex) ;
		      if (ex==0) {
			 char *uri =  r->unparsed_uri;
			 ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r,"mod_ta: added mod_ta for request  %s",uri) ;
